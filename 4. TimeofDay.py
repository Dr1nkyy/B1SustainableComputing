...
# It takes carbon intensity data for different times of the day.
...
def datetimecarbonintensity(DateTime):
    headers = {
    'Accept': 'application/json'
    }
 
    r = requests.get(f'https://api.carbonintensity.org.uk/intensity/{DateTime}', params={}, headers = headers)
    data = r.json()
    
    # index to actual value
    data = data['data'][0]['intensity']['actual']
...
def run_multiple_times(path, startDate,DTIntensity, num_runs=5):
    results_df = pd.DataFrame(columns=['Frequency', 'MSE', 'Runtime', 'Energy', 'Carbon'])
    
    for _ in range(num_runs):
        run_results = run_monitoring(path, startDate,DTIntensity)
    ...
    # Write to CSV
    results_df.to_csv('Step4/DateTime & Location/Results3pm.csv')
...
DTIntensity = '2025-01-16T22:00Z'   # carbon intensity at **:** on 16th Jan 2025. Taken from National Grid API
num_runs = 20

run_multiple_times(path, startDate,DTIntensity, num_runs)
